...
function StatsTooltip({ lineStat, match }) {
  //선수 비교 능력치 탭 툴팁
  ...
  const tooltipContents = {
    ...
    "Time Till Joining Engagement (sec)":
  ...
  const handleContent = () => {
    const contents = [];
    // const title = [];
    lineStat?.forEach((name) => {
      if (tooltipContents[name?.y]) {
        contents.push({ title: name.y, content: tooltipContents[name?.y] });
      }
    });
  ...
  return (
    <ArrowWrapper>
      <StatsTooltipWrapper>
        {content?.map((tooltip, idx) => {
          return (
            <TooltipWrapper key={idx}>
              <Title>{tooltip.title.toUpperCase()}</Title>
